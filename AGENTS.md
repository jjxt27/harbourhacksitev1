...
- **Container steps on one element must come from one scale, declared in ascending order.** Tailwind emits container variants in the order it learns them, not by value, so a custom `--container-*` step mixed with the built-in scale can emit *before* a smaller built-in and be overridden by it at every width above that. It looks exactly like the query not working. Zone three uses `--container-pair` and `--container-roomy` for this reason.
- **The registrant's email never reaches `ManifestCard`.** The card is rasterised and downloaded to be posted publicly. `ManifestData` is the printed half, `Registration` adds the address, and the card is passed the four printed fields explicitly — not the whole form state.
- **`lib/registration.ts` validates on both sides.** The browser copy is for error messages; the route re-runs it because the endpoint is public. Never trust the client's enums, and never coerce an unknown one to a default.
- **The manifest form, `/api/manifest` and `validateRegistration` are parked, not dead.** The live ask is the three-field EOI at `/eoi`. The manifest is whole and working, held for the team-forming round — do not delete it as unused code.
- **A store write sends only the fields it has.** One Airtable row is built up by both forms, merged on email. Writing a key with an empty value blanks whatever the other form put there, so `fieldsFor` omits absent fields rather than emptying them.
- **The honeypot is `company_website`, never `company`.** The EOI asks for a company for real; a trap sharing a name with a live field eventually eats genuine submissions.
...
- **The third judging criterion is `Revenue signal`, never "verified signups".** It was renamed because signups were the weakest thing a team could come back with and the wording rewarded collecting email addresses. It now reads: evidence someone outside the team acted — a sale, a signed EOI, a pilot, or hard feedback from a real prospect — with money the strongest form. Anywhere the three criteria are listed (`content/canvas.ts`, `content/intro.ts`, both event briefs, every partner brief) says the same thing. A team's EOI is a customer commitment; the site's EOI is the registration form. Don't let the two blur in copy that shows both.
