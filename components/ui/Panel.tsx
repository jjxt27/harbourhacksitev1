...
export function Panel({ children, label, meta, tone = "paper", className = "" }: PanelProps) {
  return (
    <section className={`border-2 border-ink ${TONES[tone]} ${className}`}>
      {label ? (
        <header
        ...
        >
          <h3 className="font-display text-meta font-black uppercase tracking-[0.14em]">{label}</h3>
          {meta ? <span className="font-mono text-meta uppercase tracking-[0.14em] opacity-80">{meta}</span> : null}
        </header>
      ) : null}
