# HarbourHack 2026 — Event Brief

**Status: draft. Cleared for public and sponsor sharing once the open items at the end are filled in.**

Sydney · 23–25 and 30 October 2026 · Open to anyone · [harbourhack.com](https://harbourhack.com)

---

## Don't just build. Ship.

HarbourHack is a go-to-market hackathon in Sydney, open to anyone who wants to build. Teams leave with something running, a case for it, and users who have actually signed up.

Most student hackathons end at the demo. This one ends at distribution. You will spend as much of the weekend talking to people who might use the thing as you spend building it.

## The format

Four days across two weeks. A build weekend, then a week to keep selling, then a pitch night in front of the room.

| | Date | Session |
|---|---|---|
| Day one | Friday 23 October | **Kickoff.** Teams form. Scope narrows. |
| Day two | Saturday 24 October | **Build session one.** Build in the morning, sell from lunchtime. |
| Day three | Sunday 25 October | **Build session two.** Close the loop on v1. |
| Day four | Friday 30 October | **Finalist pitch night.** Live demonstrations and judging. |

Splitting the pitch off from the build weekend is the point of the format. A team that has only ever shown its product to a judging panel has not shipped anything; a team with a week between v1 and the stage has to go and find real users, and the pitch is where they show what came back.

**Friday 23 October — Kickoff**
- Doors, registration, team forming
- Workshop — Cold outreach that gets replies
- Pick one user and write them down

**Saturday 24 October — Build session one**
- Workshop — Prototyping in Figma
- Mentor floor walk
- Ship v1 and begin outreach

**Sunday 25 October — Build session two**
- Workshop — Landing pages that convert
- Signup push
- Deck clinic

**Friday 30 October — Finalist pitch night**
- Live demonstrations
- Judging and results

Session times are TBC.

## How it's judged

Three criteria, weighted equally. Nothing else is scored.

| | Criterion | What it means |
|---|---|---|
| 01 | **Live demonstration** | The product runs, in front of the room, without a video. |
| 02 | **Commercial case** | Who it is for, what they do today, and why they would switch. |
| 03 | **Verified signups** | Real users who opted in during the event. Not your group chat. |

## Who it's for

Anyone who wants to build, across three broad disciplines — **Tech**, **Biz**, **Design**. Students, people working, people between things, people who have never entered one of these before. You can arrive with a full team or arrive alone; the Friday kickoff is set aside for teams to form. Registration asks what you're strong at and what you're looking for, so people can find each other before the doors open.

## Mentors

Operators and founders from the Sydney ecosystem, on the floor through the build weekend and available for the deck clinic. Names are published as each confirms — none are announced yet.

## For partners

HarbourHack is being built now, which is the useful part of coming in early: the programme is not yet fixed, and a partner can shape the parts that touch them. The places where a partner fits are the workshop slots, the mentor floor, the judging panel, a prize category, the venue, and the pitch night itself.

What's on the table and what it costs are both open — the conversation is the point. **Contact: TBC.**

## Registering

Expressions of interest are open at [harbourhack.com](https://harbourhack.com). Complete the manifest and you'll get a boarding pass to share, plus everything else as it's confirmed.

---

## Short version

*For reposting, socials, and society newsletters. ~95 words.*

> HarbourHack 2026 is a go-to-market hackathon in Sydney, open to anyone who wants to build, running 23–25 October with a finalist pitch night on 30 October. Most student hackathons end at the demo — this one ends at distribution. You'll spend as much of the weekend talking to people who might use your product as you spend building it, then a week to keep selling before you pitch. Judging is three things, weighted equally: a live demonstration with no video, a commercial case, and verified signups from real users. Register your interest at harbourhack.com.

## One-liner

> HarbourHack 2026 — Sydney's go-to-market hackathon, open to anyone who wants to build. 23–25 and 30 October. Don't just build. Ship.

---

## Open items

Do not fill these with plausible-looking placeholders. They go in when they're real — here and in `content/canvas.ts` at the same time.

| Item | Status |
|---|---|
| Dates | **Confirmed — 23, 24, 25 and 30 October 2026** |
| Venue | TBC |
| Session times | TBC |
| How finalists are selected between 25 and 30 October | TBC |
| Whether all teams pitch on 30 October, or only finalists | TBC |
| Mentors and judges | TBC — none confirmed |
| Prizes | TBC |
| Cost to attend | Not yet decided |
| Team size limits | Not yet decided |
| Eligibility | **Confirmed — open to anyone** |
| Application deadline / capacity | Not yet decided |
| Contact address for enquiries and partners | Not yet decided |

**Deliberately not claimed anywhere in this brief:** attendee numbers, sponsor names, past-event history, prize values, judge names. HarbourHack 2026 has no track record to cite yet, and inventing one is the fastest way to lose the room. A sponsor deck will eventually need audience numbers — those should be a stated projection with the reasoning attached, never presented as fact.
